...
* DataPreprocessing
  ...
  >
  > `data_to_csv.ipynb`: Obtain the gene expression matrix after cell filtering and highly variable gene selection, along with the data splits for five-fold cross-validation. Save them as CSV files for network construction using both WGCNA and PCA-PMI methods. The processed data is saved in the `dataset/pre_data/scRNAseq_datasets_hvgs`directory.
