...
from datasets_grnboost2 import MyDataset
...
def get_parser(parser=None):
    ...
    parser.add_argument('-netname','--netname',type=str, default='grnboost2')    # different network construction method
...
def classify_test(args):
    ...
    models_folder = pathjoin(seq_folder, 'grnboost2_models')
    os.makedirs(pathjoin(save_folder, 'grnboost2_preds'),exist_ok=True)
    preds_folder = pathjoin(save_folder, 'grnboost2_preds')
    ...
    for k in range(n_splits):
        ...
        net_df = pd.read_csv(net_file, sep='\t', header=None, dtype={0: str, 1: str})
        gene_name_to_index = dict(zip(gene_symbol[filtered_genes_index], range(len(filtered_genes_index))))

        net_df['gene1'] = net_df[0].map(gene_name_to_index).astype(int)  
        net_df['gene2'] = net_df[1].map(gene_name_to_index).astype(int)  
