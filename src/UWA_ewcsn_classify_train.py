...
def get_parser(parser=None):
    ...
    parser.add_argument('-outdir', type=str, default='result/models_UWA')  # Output directory
...
def classify_train(args):
    """
    Train a classification model using scRNA-seq data and EWCSN(UMA).
    ...
    # Process network file name, e.g., HumanNet-GSP
    ...
    for k in range(n_splits):
        ...
        # Get submatrix of network adjacency
        cur_net_adj = net_adj.tocsr()[filtered_genes_index, :][:, filtered_genes_index].tocoo()  
        
        # Create train dataset and dataloader, get EWCSN(UWA)
        train_dataset = MyDataset2(root=cell_train_folder, my_indices=train_index_imputed[k], net_adj=cur_net_adj)
